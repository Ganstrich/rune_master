...
class ParameterTuner:
    ...
    def __init__(self, equipments: List[Equipment], cache_manager=None, api_client=None):
    ...
    def evaluate_quality(stats: Dict[str, Any]) -> float:
        ...
        score = (retention * 0.3) + (efficiency * 0.4) + (size_score * 0.3) - mega_penalty
        return max(0.0, score)

    def tune(self, method: str = "deterministic") -> Tuple[ProcessingConfig, Dict[str, Any]]:
        ...
        cache_file = self.cache_manager.cache_file if self.cache_manager else "resource_cache.json"

        # Execute combinations in parallel
        with concurrent.futures.ProcessPoolExecutor() as executor:
            futures = [
                executor.submit(_worker_run_config, self.equipments, r, c, method, cache_file)
            ...
            for i, future in enumerate(concurrent.futures.as_completed(futures), 1):
                try:
                    ...
                    print(f"      [{i}/{len(combinations)}] Ratio: {ratio:.2f}, MinItems: {count} => Score: {score:.2f} (Ret: {retention:.1%}, MaxSize: {max_s})")
